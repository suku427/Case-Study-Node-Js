...
// This is the new function for the dashboard analytics
const getProductsDashboard = async (req, res) => {
  try {
    const { startDate, endDate, bucket } = req.query;

    // 1. Get the current total number of products
    const currentTotal = await prisma.product.count();

    // 2. Fetch the historical trend data from the product_trends table
    let trendData;
    const query = {
      where: {},
    ...
    if (startDate) {
      query.where.trend_date = { ...query.where.trend_date, gte: new Date(startDate) };
    }
    if (endDate) {
      query.where.trend_date = { ...query.where.trend_date, lte: new Date(endDate) };
    }

    trendData = await prisma.product_trends.findMany(query);
    // 3. For now, let's assume the bucket is 'day' to keep it simple.
    const formattedTrend = trendData.map(data => ({
